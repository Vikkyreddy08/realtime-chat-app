...
import { Search, Plus, MessageSquare, Users, LogOut, User, X } from 'lucide-react'
...
const Sidebar = () => {
  ...
  const handleCreateGroup = async () => {
    if (selectedGroupUsers.length < 2 || !groupName.trim()) return
    try {
      const room = await createRoom(selectedGroupUsers.map(u => u.id), 'group', groupName.trim())
      setCurrentRoom(room)
      ...
      <div className="p-4 border-b border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-2">
            <Link
              to="/profile"
              className="p-2 text-gray-500 hover:text-blue-500 rounded-full hover:bg-gray-100 dark:hover:bg-gray-700 transition-all"
              title="Profile"
            >
              <User size={20} />
            </Link>
            <button
              onClick={logout}
              className="p-2 text-gray-500 hover:text-red-500 rounded-full hover:bg-gray-100 dark:hover:bg-gray-700 transition-all"
              title="Logout"
            >
              <LogOut size={20} />
            </button>
          ...
          <div>
            ...
            <div className="px-2 mb-4">
              <input
                ...
                className="w-full px-3 py-2 bg-gray-100 dark:bg-gray-700 text-gray-900 dark:text-white rounded-lg mb-3"
              />
              {selectedGroupUsers.length > 0 && (
                <div className="flex flex-wrap gap-2 mb-3">
                  {selectedGroupUsers.map(u => (
                    <div
                      key={u.id}
                      className="flex items-center gap-1 bg-blue-100 dark:bg-blue-900 px-2 py-1 rounded-full text-xs text-blue-700 dark:text-blue-300"
                    >
                      {u.username}
                      <button
                        onClick={(e) => {
                          e.stopPropagation()
                          setSelectedGroupUsers(prev => prev.filter(x => x.id !== u.id))
                        }}
                      >
                        <X size={12} />
                      </button>
              ...
              <button
                ...
                className="w-full py-2 bg-purple-500 hover:bg-purple-600 disabled:bg-gray-300 disabled:cursor-not-allowed text-white font-semibold rounded-lg"
              >
                Create Group
              </button>
            </div>
            ...
            {filteredUsers.map(u => (
              <div
                key={u.id}
                onClick={() => {
                  if (selectedGroupUsers.some(x => x.id === u.id)) {
                    setSelectedGroupUsers(prev => prev.filter(x => x.id !== u.id))
                  } else {
                ...
                className={`flex items-center gap-3 p-3 rounded-xl cursor-pointer transition-all mb-1 ${
                  selectedGroupUsers.some(x => x.id === u.id)
                    ? 'bg-purple-50 dark:bg-purple-900/30 ring-1 ring-purple-200 dark:ring-purple-800'
                    : 'hover:bg-gray-100 dark:hover:bg-gray-700'
                }`}
              >
                <div className="relative">
                  <div className="w-11 h-11 rounded-full bg-gradient-to-br from-purple-500 to-blue-500 flex items-center justify-center text-white text-sm font-semibold">
                    {u.username.charAt(0).toUpperCase()}
                  </div>
                  <div className={`absolute bottom-0 right-0 w-3 h-3 rounded-full border-2 border-white dark:border-gray-800 ${
                    u.online ? 'bg-green-500' : 'bg-gray-400'
